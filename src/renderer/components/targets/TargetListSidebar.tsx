...
export function TargetListSidebar() {
    const { lists, selectedListId, setSelectedListId, addList, deleteList, updateList, isLoading, viewMode, setViewMode } = useTargetsStore();
            ...
            <div className="flex flex-col h-full w-full" style={{ width: `${width}px` }}>
                <div className="p-5 border-b border-border/5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        {!isPro() && (
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-muted/30 text-muted-foreground/50 border border-border/20">
                                {lists.length}/3
                            </span>
                        )}
                    </div>
                        ...
                        {lists.map((list) => (
                            <div key={list.id} className="min-w-0">
                                {renamingId === list.id ? (
                                    ...
                                    <div
                                        className={cn(
                                            "group flex items-center px-4 py-2 rounded-xl transition-all cursor-pointer min-w-0 border border-transparent",
