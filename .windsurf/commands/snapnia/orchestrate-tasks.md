...
In this spec's folder, create this file: `snapnia/specs/[this-spec]/orchestration.yml`.
...
Loop through each task group in `snapnia/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned subagent specified in `orchestration.yml`.

For each delegation, provide the subagent with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `snapnia/specs/[this-spec]/spec.md`
- Instruct subagent to:
  - Perform their implementation
  - Check off the task and sub-task(s) in `snapnia/specs/[this-spec]/tasks.md`
{{UNLESS standards_as_claude_code_skills}}
...
Follow these steps to generate this spec's ordered series of prompts texts, each in its own .md file located in `snapnia/specs/[this-spec]/implementation/prompts/`.

LOOP through EACH task group in `snapnia/specs/[this-spec]/tasks.md` and for each, use the following workflow to generate a markdown file with prompt text for each task group:

#### Step 1. Create the prompt markdown file

Create the prompt markdown file using this naming convention:
`snapnia/specs/[this-spec]/implementation/prompts/[task-group-number]-[task-group-title].md`.
...
Read @snapnia/specs/[this-spec]/spec.md to understand the context for this spec and where the current task fits into it.

Also read these further context and reference:
- @snapnia/specs/[this-spec/]/planning/requirements.md
- @snapnia/specs/[this-spec/]/planning/visuals
...
Progress will be tracked in `snapnia/specs/[this-spec]/tasks.md`
```
